...
const FeatureJobs = ({ job }) => {
    ...
    return (
        <div style={{ borderRadius: "8px", border: "1px solid var(--Dark-06, #E8E8E8)" }} className="py-10 px-10" key={job.id} >
            ...
            <h3 style={{ color: "var(--Dark-02, #474747)" }} className="text-2xl font-extrabold mb-3">{job.job_title}</h3>
            <p style={{ color: "var(--Dark-03, #757575)" }} className="text-xl mb-4">{job.company_name}</p>
            ...
            <p style={{ color: "var(--Dark-03, #757575)" }} className='flex gap-4 mb-4'>
            ...
            <button className='px-5 py-3 text-xl text-white font-extrabold ' style={{ borderRadius: '4px', background: "linear-gradient(90deg, #7E90FE 0%, #9873FF 100%)" }}>View Details</button>
        </div >
    );
};
